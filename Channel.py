...
class Channel:
    ...
    def calculate_channel_dimensions(self):
        ...
        excavation_height=excavation_width*np.tan(self.channel_data["terrain slope"])
        ...
        excavation_vol=self.channel_data["channel length"]*(excavation_width*(foundation_thickness+0.1)+np.power(excavation_width, 2) * 0.5 * np.tan(np.deg2rad(self.channel_data["terrain slope"])))
    ...
    def calculate_channel_labour(self):
        self.channel_cost["excavation labour"] = (self.channel_dimensions["excavation_vol"] * (1.1123*np.exp(0.4774*self.labour_time["excavating_factor"]))) * self.labour_cost["noskill_worker"]
